  ...
  , _geometryName()
{
  ...
  // and geometry data
  RBDL::Model model;
  if (!RBDL::Addons::URDFReadFromFile(filename.c_str(), &model, false, &_inertiaData, &_inertiaName, false,
                                      &_geometryData, &_geometryName, false))
  {
    throw std::runtime_error("Model unable to load URDF file: " + filename);
  }

  // Parse and load RBDL model
  initializeModel(model, _inertiaData, _inertiaName, _geometryData, _geometryName);
}

Model::Model(const std::string& filename, const Eigen::MatrixXd& inertiaData,
  ...
  , _geometryName(geometryName)
{
  ...
  // and geometry data
  RBDL::Model model;
  if (!RBDL::Addons::URDFReadFromFile(filename.c_str(), &model, false, &_inertiaData, &_inertiaName, true,
...
Model::Model(RBDL::Model& model, const Eigen::MatrixXd& inertiaData, const std::map<std::string, size_t>& inertiaName,
...
void Model::updateDOFPosition()
{
  if (!_isAutoUpdate)
  {
    RBDL::UpdateKinematicsCustom(_model, &_dofs, nullptr, nullptr);
  }
}
...
void Model::setDOFVect(const Eigen::VectorXd& vect)
{
  ...
  {
    throw std::logic_error("Model invalid DOF vector size");
  }
  _dofs = vect;
}
...
void Model::importDOF(Model& model)
{
  ...
  {
    _dofs(i) = model._dofs(model._dofNameToIndex.at(_dofIndexToName.at(i)));
  }
}
...
Eigen::Vector3d Model::position(size_t srcFrameIndex, size_t dstFrameIndex, const Eigen::Vector3d& point)
{
  ...
  if (srcFrameIndex != 0)
  {
    ptBase = RBDL::CalcBodyToBaseCoordinates(_model, _dofs, srcFrameIndex, point, _isAutoUpdate);
  }
  else
  {
    ptBase = point;
  ...
  if (dstFrameIndex != 0)
  {
    ptBody = RBDL::CalcBaseToBodyCoordinates(_model, _dofs, dstFrameIndex, ptBase, _isAutoUpdate);
  }
  else
  {
    ptBody = ptBase;
...
Eigen::Vector3d Model::position(const std::string& srcFrame, const std::string& dstFrame, const Eigen::Vector3d& point)
{
  return position(getFrameIndex(srcFrame), getFrameIndex(dstFrame), point);
}

Eigen::Matrix3d Model::orientation(size_t srcFrameIndex, size_t dstFrameIndex)
{
  // Convert to body id
  srcFrameIndex = _frameIndexToId.at(srcFrameIndex);
  dstFrameIndex = _frameIndexToId.at(dstFrameIndex);
  ...
  transform1 = CalcBodyWorldOrientation(_model, _dofs, srcFrameIndex, _isAutoUpdate);
  RBDLMath::Matrix3d transform2;
  transform2 = CalcBodyWorldOrientation(_model, _dofs, dstFrameIndex, _isAutoUpdate);

  return transform1 * transform2.transpose();
}
Eigen::Matrix3d Model::orientation(const std::string& srcFrame, const std::string& dstFrame)
{
  return orientation(getFrameIndex(srcFrame), getFrameIndex(dstFrame));
}
...
Eigen::Affine3d Model::getTransform(const std::string& srcFrame, const std::string& dstFrame)
{
  return getTransform(getFrameIndex(srcFrame), getFrameIndex(dstFrame));
}

double Model::orientationYaw(size_t srcFrameIndex, size_t dstFrameIndex)
{
  Eigen::Matrix3d rotation = orientation(srcFrameIndex, dstFrameIndex);
  rotation.transposeInPlace();

  return atan2(rotation(1, 0), rotation(0, 0));
}
double Model::orientationYaw(const std::string& srcFrame, const std::string& dstFrame)
{
...
Eigen::MatrixXd Model::pointJacobian(const std::string& srcFrame, const std::string& dstFrame,
                                     const Eigen::Vector3d& point)
{
  ...
  // world origin frame
  CalcPointJacobian6D(_model, _dofs, srcFrameIndex, point, G, true);
...
Eigen::VectorXd Model::pointVelocity(const std::string& pointFrame, const std::string& dstFrame,
                                     const Eigen::VectorXd& velocity, const Eigen::Vector3d& point)
{
  // Convert to body id
  ...
  // Compute velocity
  Eigen::VectorXd vel = CalcPointVelocity6D(_model, _dofs, velocity, pointFrameIndex, point, true);
...
Eigen::VectorXd Model::pointAcceleration(const std::string& pointFrame, const std::string& dstFrame,
                                         ...
                                         const Eigen::Vector3d& point)
{
  ...
  Eigen::VectorXd acc = CalcPointAcceleration6D(_model, _dofs, velocity, acceleration, pointFrameIndex, point, true);
...
Eigen::Vector3d Model::centerOfMass(size_t frameIndex)
{
  ...
  RBDL::Utils::CalcCenterOfMass(_model, _dofs, _dofs, mass, com, nullptr, nullptr, _isAutoUpdate);
...
double Model::sumMass()
{
  ...
  double mass;
  RBDLMath::Vector3d com;
  RBDL::Utils::CalcCenterOfMass(_model, Q, Q, mass, com, nullptr, nullptr, _isAutoUpdate);
...
Eigen::VectorXd Model::inverseDynamics(const Eigen::VectorXd& velocity, const Eigen::VectorXd& acceleration)
{
  ...
  {
    throw std::logic_error("Model invalid velocity vector size");
  }
  else
  {
    QDot = velocity;
  ...
  {
    throw std::logic_error("Model invalid acceleration vector size");
  }
  else
  {
    QDDot = acceleration;
  ...
  tau.setZero();
  RBDL::InverseDynamics(_model, _dofs, QDot, QDDot, tau, NULL);

  return tau;
}
VectorLabel Model::inverseDynamics(const VectorLabel& velocity, const VectorLabel& acceleration)
{
...
Eigen::VectorXd Model::inverseDynamicsClosedLoop(size_t fixedFrameIndex, Eigen::VectorXd* contactForce,
                                                 bool useInfinityNorm, const Eigen::VectorXd& velocity,
                                                 const Eigen::VectorXd& acceleration)
{
  ...
  return RBDLClosedLoopInverseDynamics(_model, _dofs, QDot, QDDot, fixedFrameId, contactForce, useInfinityNorm);
}
Eigen::VectorXd Model::inverseDynamicsClosedLoop(const std::string& fixedFrameName, Eigen::VectorXd* contactForce,
                                                 bool useInfinityNorm, const Eigen::VectorXd& velocity,
                                                 const Eigen::VectorXd& acceleration)
{
  return inverseDynamicsClosedLoop(getFrameIndex(fixedFrameName), contactForce, useInfinityNorm, velocity,
                                   acceleration);
}

Eigen::VectorXd Model::forwardDynamics(const Eigen::VectorXd& position, const Eigen::VectorXd& velocity,
                                       const Eigen::VectorXd& torque)
{
  // Sanity check
  if (position.size() != _model.dof_count)
  {
    throw std::logic_error("Model invalid position vector size");
  }
  ...
  RBDL::ForwardDynamics(_model, position, velocity, torque, QDDot, NULL);

  return QDDot;
}

Eigen::VectorXd Model::forwardDynamicsCustom(const Eigen::VectorXd& position, const Eigen::VectorXd& velocity,
                                             ...
                                             RBDLMath::LinearSolver solver)
{
  ...
  {
    throw std::logic_error("Model invalid inertia vector size");
  }
  ...
  Eigen::VectorXd acceleration = Eigen::VectorXd::Zero(sizeDOF);

  // Compute full H anc C matrix
  RBDLMath::MatrixNd H = RBDLMath::MatrixNd::Zero(sizeDOF, sizeDOF);
  RBDLMath::VectorNd C = RBDLMath::VectorNd::Zero(sizeDOF);
  // Compute C with inverse dynamics
  acceleration.setZero();
  RBDL::InverseDynamics(_model, position, velocity, acceleration, C, NULL);
  // Compute H
  RBDL::CompositeRigidBodyAlgorithm(_model, position, H, false);
...
Eigen::VectorXd Model::forwardDynamicsContacts(RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position,
                                               const Eigen::VectorXd& velocity, const Eigen::VectorXd& torque)
{
  ...
  RBDLMath::VectorNd QDDot(_model.dof_count);
  RBDL::ForwardDynamicsContactsDirect(_model, position, velocity, torque, constraints, QDDot);

  return QDDot;
}

Eigen::VectorXd Model::forwardDynamicsContactsCustom(RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position,
                                                     const Eigen::VectorXd& velocity, const Eigen::VectorXd& torque,
                                                     ...
                                                     RBDLMath::LinearSolver solver)
{
  ...
  if (torque.size() != _model.dof_count)
  {
    throw std::logic_error("Model invalid torque vector size");
  }
  if (inertiaOffset.size() != _model.dof_count)
  {
    throw std::logic_error("Model invalid inertia vector size");
  ...
  RBDL::CalcContactSystemVariables(_model, position, velocity, torque, constraints);
  // Add inertial diagonal offsets
  for (size_t i = 0; i < (size_t)inertiaOffset.size(); i++)
  {
  ...
  constraints.A.block(0, 0, sizeDOF, sizeDOF) = constraints.H;
  constraints.A.block(sizeDOF, 0, sizeCst, sizeDOF) = constraints.G;
  constraints.A.block(0, sizeDOF, sizeDOF, sizeCst) = constraints.G.transpose();
  constraints.b.segment(0, sizeDOF) = torque - constraints.C;
  constraints.b.segment(sizeDOF, sizeCst) = constraints.gamma;

  // Solve the linear system
  switch (solver)
  {
    case RBDLMath::LinearSolverPartialPivLU:
      constraints.x = constraints.A.partialPivLu().solve(constraints.b);
      break;
    case RBDLMath::LinearSolverColPivHouseholderQR:
      constraints.x = constraints.A.colPivHouseholderQr().solve(constraints.b);
      break;
    case RBDLMath::LinearSolverHouseholderQR:
      constraints.x = constraints.A.householderQr().solve(constraints.b);
      break;
    case RBDLMath::LinearSolverFullPivLU:
      constraints.x = constraints.A.fullPivLu().solve(constraints.b);
      break;
    case RBDLMath::LinearSolverFullPivHouseholderQR:
      constraints.x = constraints.A.fullPivHouseholderQr().solve(constraints.b);
      break;
    default:
      assert(0);
      break;
...
Eigen::VectorXd Model::forwardImpulseDynamicsContactsCustom(
    double dt, RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position, const Eigen::VectorXd& velocity,
    const Eigen::VectorXd& torque, const Eigen::VectorXd& inertiaOffset, RBDLMath::LinearSolver solver)
{
  // Sanity check
  if (position.size() != _model.dof_count)
  {
  ...
  // vectors into the constraint set
  RBDL::CalcContactSystemVariables(_model, position /*+ dt*velocity TODO XXX ??? usefull*/, velocity, torque,
                                   constraints);
  // Add inertial diagonal offsets
  for (size_t i = 0; i < (size_t)inertiaOffset.size(); i++)
  {
  ...
  if (!HQR.isInvertible())
  {
    throw std::logic_error("RBDLContactLCP non invertible H matrix");
  }
  ...
  */
  ...
  constraints.A.block(0, 0, sizeDOF, sizeDOF) = constraints.H;
  constraints.A.block(sizeDOF, 0, sizeCst, sizeDOF) = constraints.G;
  constraints.A.block(0, sizeDOF, sizeDOF, sizeCst) = -dt * constraints.G.transpose();
  constraints.b.segment(0, sizeDOF) = dt * (torque - constraints.C) + constraints.H * velocity;
...
Eigen::VectorXd Model::inverseDynamicsContacts(RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position,
                                               const Eigen::VectorXd& velocity, const Eigen::VectorXd& acceleration,
                                               const Eigen::VectorXd& inertiaOffset)
{
  // Sanity check
  ...
  // vectors into the constraint set
  //(actually, torque is not used by RBDL)
  RBDL::CalcContactSystemVariables(_model, position, velocity, Eigen::VectorXd::Zero(sizeDOF), constraints);
  // Add inertial diagonal offsets
  for (size_t i = 0; i < (size_t)inertiaOffset.size(); i++)
  {
    constraints.H(i, i) += inertiaOffset(i);
  ...
  constraints.b.setZero();
  constraints.A.block(0, 0, sizeDOF, sizeDOF) = constraints.H;
  constraints.A.block(sizeDOF, 0, sizeConstraints, sizeDOF) = constraints.G;
  constraints.A.block(0, sizeDOF, sizeDOF, sizeConstraints) = constraints.G.transpose();
  constraints.b.segment(0, sizeDOF) = acceleration;
  constraints.b.segment(sizeDOF, sizeConstraints) = -constraints.force;
...
Eigen::VectorXd Model::impulseContacts(RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position,
                                       const Eigen::VectorXd& velocity)
{
  ...
  RBDL::ComputeContactImpulsesDirect(_model, position, velocity, constraints, newVel);

  return newVel;
}

Eigen::VectorXd Model::impulseContactsCustom(RBDL::ConstraintSet& constraints, const Eigen::VectorXd& position,
                                             const Eigen::VectorXd& velocity, const Eigen::VectorXd& inertiaOffset,
                                             RBDLMath::LinearSolver solver)
{
  // Sanity check
  if (position.size() != _model.dof_count)
  ...
  RBDL::UpdateKinematicsCustom(_model, &position, NULL, NULL);
  RBDL::CompositeRigidBodyAlgorithm(_model, position, constraints.H, false);
  // Add inertial diagonal offsets
  for (size_t i = 0; i < (size_t)inertiaOffset.size(); i++)
  {
    constraints.H(i, i) += inertiaOffset(i);
  ...
  // Compute G
  RBDL::CalcContactJacobian(_model, position, constraints, constraints.G, false);
  ...
  constraints.A.block(sizeDOF, 0, sizeCst, sizeDOF) = constraints.G;
  constraints.A.block(0, sizeDOF, sizeDOF, sizeCst) = constraints.G.transpose();
  constraints.b.segment(0, sizeDOF) = constraints.H * velocity;
  constraints.b.segment(sizeDOF, sizeCst) = constraints.v_plus;
...
void Model::initializeModel(RBDL::Model& model, const Eigen::MatrixXd& inertiaData,
                            const std::map<std::string, size_t>& inertiaName, const Eigen::MatrixXd& geometryData,
                            const std::map<std::string, size_t>& geometryName)
{
  // Assign RBDL model
  ...
  for (size_t i = 1; i < _model.mBodies.size(); i++)
  {
    ...
    std::string filteredName = filterJointName(getRBDLBodyName(i, virtualDepth));
    ...
    else if (virtualDepth > 0)
    {
      ...
      throw std::logic_error("Model virtual body name not implemented: name=" + filteredName + std::string(" depth=") +
                             std::to_string(virtualDepth) + std::string("\n") + std::string("ModelHierarchy:\n") +
...
std::string Model::getRBDLBodyName(size_t bodyId, unsigned int& virtualDepth) const
{
...
void Model::loadLabelToEigen(const VectorLabel& vect, Eigen::VectorXd& dst, bool setBase)
{
  for (size_t i = 0; i < vect.size(); i++)
  {
    const std::string& label = vect.getLabel(i);
    if (_dofNameToIndex.count(label) != 0 && (setBase || label.find("base_") == std::string::npos))
    {
