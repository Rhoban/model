...
public:
   ...
   */
  HumanoidModel(RobotType type, const std::string& frameRoot, bool isFloatingBase = true,
   ...
   */
  HumanoidModel(const std::string& urdfFile, RobotType type, const std::string& frameRoot, bool isFloatingBase = true,
                const Eigen::MatrixXd& inertiaData = Eigen::MatrixXd(),
                const std::map<std::string, size_t>& inertiaName = {},
                const Eigen::MatrixXd& geometryData = Eigen::MatrixXd(),
                const std::map<std::string, size_t>& geometryName = {});
   ...
   */
  void initialize(const std::string& urdfFile, RobotType type, const std::string& frameRoot, bool isFloatingBase = true,
                  const Eigen::MatrixXd& inertiaData = Eigen::MatrixXd(),
                  const std::map<std::string, size_t>& inertiaName = {},
                  const Eigen::MatrixXd& geometryData = Eigen::MatrixXd(),
                  const std::map<std::string, size_t>& geometryName = {});
   ...
   */
  void boundingBox(size_t frameIndex, double& sizeX, double& sizeY, double& sizeZ,
   ...
   */
  bool legIkLeft(const std::string& frame, const Eigen::Vector3d& footPos,
                 const Eigen::Matrix3d& rotation = Eigen::Matrix3d::Identity(), double* boundIKDistance = nullptr);
  bool legIkRight(const std::string& frame, const Eigen::Vector3d& footPos,
                  const Eigen::Matrix3d& rotation = Eigen::Matrix3d::Identity(), double* boundIKDistance = nullptr);
   ...
   */
  Eigen::Vector3d selfInFrame(const std::string& name, const Eigen::Vector3d& pos = Eigen::Vector3d::Zero());
  Eigen::Vector3d frameInSelf(const std::string& name, const Eigen::Vector3d& pos = Eigen::Vector3d::Zero());
   ...
   */
  Eigen::Vector3d cameraPixelToViewVector(const CameraModel& cameraModel, const Eigen::Vector2d& pixel);
  Eigen::Vector3d cameraPanTiltToViewVector(const Eigen::Vector2d& anglesPanTilt);
   ...
   */
  bool cameraViewVectorToWorld(const Eigen::Vector3d& viewVector, Eigen::Vector3d& pos, double groundHeight = 0.0);
   ...
   */
  bool getImgRadiusFromViewVector(const CameraModel& cameraModel, const Eigen::Vector3d& viewVector, double ballRadius,
   ...
   */
  Eigen::Vector2d cameraViewVectorToPanTilt(const Eigen::Vector3d& viewVector);
   ...
   */
  Eigen::Vector2d cameraPixelToPanTilt(const CameraModel& cameraModel, const Eigen::Vector2d& pixel,
   ...
   */
  bool cameraWorldToPixel(const CameraModel& cameraModel, const Eigen::Vector3d& pos, Eigen::Vector2d& pixel);
   ...
   */
  bool cameraPanTiltToPixel(const CameraModel& cameraModel, const Eigen::Vector2d& anglesPanTilt,
  ...
  bool cameraLookAtNoUpdate(double& panDOF, double& tiltDOF, const Eigen::Vector3d& posTarget);
   ...
   * between -1 and 1.
   */
  double cameraScreenHorizon(const CameraModel& cameraModel, double screenPosWidth);
...
private:
   ...
   */
  LegIK::Vector3D buildTargetPos(const std::string& frame, const Eigen::Vector3d& footPos, bool isLeftLeg);
  LegIK::Frame3D buildTargetOrientation(const std::string& frame, const Eigen::Matrix3d& rotation);

  /**
   * Assign model leg DOF to given IK results
   */
  void setIKResult(const LegIK::Position& result, bool isLeftLeg);

  /**
   * Check inverse kinematics computed value
   * and throw an error in case of NaN
   */
  void checkNaN(const LegIK::Position& result, const LegIK::Vector3D& pos, const LegIK::Frame3D& orientation) const;
};
