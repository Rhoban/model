...
int main()
{
  ...
  while (viewer.update())
  {
    ...
    model.get().cameraViewVectorToWorld(
        model.get().cameraPixelToViewVector(camParams, Eigen::Vector2d(0.0, pixelTarget)), groundPos);
    ...
    bool success1 = model.get().cameraViewVectorToWorld(model.get().cameraPixelToViewVector(camParams, pixel), ground);
    ...
    model.get().cameraViewVectorToBallWorld(camParams,
                                            ...
                                            0.07, ballCenter, &ballCenterPixel, &bordersPixel, &borders);
    ...
    Eigen::Vector2d angles = model.get().cameraPixelToPanTilt(camParams, pixelTarget2);
    ...
    plot.add(Leph::VectorLabel("head_yaw", model.get().getDOF("head_yaw"), "head_pitch",
                               model.get().getDOF("head_pitch"), "time", t));
  }
