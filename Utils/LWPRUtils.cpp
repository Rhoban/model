...
LWPR_Object LWPRInit(size_t inputDim, const Eigen::VectorXd& rawParams)
{
...
Eigen::VectorXd LWPROptimizeParameters(size_t inputDim, const Eigen::VectorXd& params,
                                       ...
                                       const std::vector<double> testOutputs, unsigned int maxIteration, bool isQuiet)
{
  // Checks
  ...
  if (trainInputs.size() != trainOutputs.size() || testInputs.size() != testOutputs.size())
  {
    throw std::logic_error("LWPRUtils invalid train or test size");
  }
  if ((size_t)trainInputs.front().size() != inputDim || (size_t)testInputs.front().size() != inputDim)
  {
  ...
  // Fitness function
  libcmaes::FitFunc fitness = [&inputDim, &trainInputs, &trainOutputs, &testInputs, &testOutputs](const double* x,
                                                                                                  const int N) {
    // Conversion to Eigen
...
void LWPRPrint(const LWPR_Object& model)
{
  ...
  for (size_t i = 0; i < (size_t)model.numRFS(); i++)
  {
    ...
    countRFTrustworthy++;
    sumRFReg += rf.nReg();
    if (minRFReg > (size_t)rf.nReg())
      minRFReg = rf.nReg();
    if (maxRFReg < (size_t)rf.nReg())
      maxRFReg = rf.nReg();
    sumRFVIP += rf.vip();
  ...
  std::cout << "RF mean nReg: " << (double)sumRFReg / (double)countRFTrustworthy << std::endl;
  for (size_t i = 0; i < (size_t)model.nIn(); i++)
  {
...
size_t LWPRMinRFReg(const LWPR_Object& model)
{
  ...
  for (size_t i = 0; i < (size_t)model.numRFS(); i++)
  {
    ...
    if (minRFReg == (size_t)-1 || minRFReg > (size_t)rf.nReg())
    {
      minRFReg = rf.nReg();
    }
  }
...
size_t LWPRMaxRFReg(const LWPR_Object& model)
{
  ...
  for (size_t i = 0; i < (size_t)model.numRFS(); i++)
  {
    ...
    if (maxRFReg == (size_t)-1 || maxRFReg < (size_t)rf.nReg())
    {
      maxRFReg = rf.nReg();
    }
  }
...
void LWPRPrintParameters(size_t inputDim, const Eigen::VectorXd& params)
{
  ...
  std::cout << "NormIn (nIn): " << params.segment(0, inputDim).transpose() << std::endl;
  ...
  std::cout << "InitD (nIn): " << params.segment(inputDim + 3, inputDim).transpose() << std::endl;
