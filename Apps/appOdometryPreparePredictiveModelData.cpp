...
int main(int argc, char** argv)
{
  ...
  {
    std::cout << "Usage: ./app "
              << "input.data odometry.conf output.data" << std::endl;
    return 1;
  }
  ...
  std::cout << "Loading odometry data from: " << inputPath << " with " << logs.size() << " sequences" << std::endl;
  ...
  for (const Leph::OdometrySequence& seq : logs)
  {
    ...
    final_pos = simulateOdometry(seq, true, odom, Leph::OdometryType::OdometryGoal, engine, &positions);
