...
int main(int argc, char** argv)
{
  ...
  logs.plot().plot("time:timestamp", "goal:*").plot("time:timestamp", "pos:*").render();
  ...
  while (viewer.update())
  {
    ...
    // Assiging pressure
    modelMotors.setPressure(logs[indexLog]("pressure:weight"), logs[indexLog]("pressure:left_ratio"),
    ...
    modelMotors.setOrientation(
        ...
        false);
    modelMotors.setYaw(modelMotors.getSupportFoot(), logs[indexLog]("sensor:gyro_yaw") - logs[0]("sensor:gyro_yaw"));
    std::cout << modelMotors.get().position("trunk", "origin").x() << " "
