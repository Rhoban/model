...
Leph::HumanoidFixedModel initModel(const Eigen::VectorXd& params)
{
  ...
  Leph::HumanoidFixedModel model(Leph::SigmabanModel, Eigen::MatrixXd(), {}, geometryData, defaultGeometryName);
 ...
 */
Eigen::VectorXd evaluateParameters(const Eigen::VectorXd& params, const Leph::VectorLabel& data, bool noRandom,
                                   Leph::HumanoidFixedModel& model, std::default_random_engine& engine)
{
  // Parse parameters
  ...
  Eigen::Vector3d viewVectorInWorld = model.get().cameraPixelToViewVector(camParams, pixelPoint);
  ...
  bool isSuccess = model.get().cameraViewVectorToWorld(viewVectorInWorld, groundEstimation, data("ground_z"));
...
int main(int argc, char** argv)
{
  ...
  std::cout << "Using a camera width of " << camParams.widthAperture * 180.0 / M_PI << "°, and a camera height of "
            << camParams.heightAperture * 180.0 / M_PI << std::endl;

  // Loading data
  Leph::MatrixLabel logs;
  logs.load(logPath);
  std::cout << "Loading data from " << logPath << ": " << logs.size() << " points" << std::endl;
  ...
  if (modelPath == "")
  {
    ...
    for (const std::string& name : Leph::NamesDOF)
    {
      ...
      jointData.block(tmpIndex, 0, 1, jointParams.size()) = jointParams.transpose();
      tmpIndex++;
    }
  }
  else
  {
    ...
    Leph::ReadModelParameters(modelPath, jointData, jointName, inertiaData, inertiaName, defaultGeometryData,
                              defaultGeometryName);
  }
  ...
  if (seedPath != "")
  {
    ...
    std::map<std::string, size_t> tmpGeometryName = Leph::ReadMapFromStream<std::string, size_t>(file);
    file.close();
  ...
  std::cout << "Writing geometry and camera data to: " << camParamsPath << std::endl;
  ...
  Leph::WriteEigenMatrixToStream(file, getGeometryDataFromParameters(bestParams));
  ...
  Leph::WriteEigenMatrixToStream(camfile, getGeometryDataFromParameters(bestParams));
