...
Eigen::Vector3d sampleObservationNoise(const Eigen::VectorXd& noiseParameters, std::default_random_engine* engine)
{
  ...
  // Generating noise
  return Eigen::Vector3d(posErrNorm * cos(posErrArg), posErrArg * sin(posErrArg), Leph::AngleBound(thetaErr));
}

Eigen::VectorXd buildObservation(const Eigen::Vector3d& final_state, LearningConfig* conf)
{
  int output_dim = 0;
  if (conf->pos_eval_type != PosEvalType::None)
    output_dim += 2;
...
RunType parseRunType(std::istream& in)
{
  ...
  std::getline(in, line);
  if (line == "Diagnose")
    return RunType::Diagnose;
  if (line == "Classic")
    return RunType::Classic;
  if (line == "Ascending")
    return RunType::Ascending;
...
PosEvalType parsePosEvalType(std::istream& in)
{
  ...
  std::getline(in, line);
  if (line == "Cartesian")
    return PosEvalType::Cartesian;
  if (line == "Polar")
    return PosEvalType::Polar;
  if (line == "None")
    return PosEvalType::None;
...
Leph::OdometryType parseOdometryType(std::istream& in)
{
  std::string line;
  std::getline(in, line);
  if (line == "OdometryOrder")
    return Leph::OdometryType::OdometryOrder;
  if (line == "OdometryGoal")
    return Leph::OdometryType::OdometryGoal;
  if (line == "OdometryRead")
    return Leph::OdometryType::OdometryRead;
...
Leph::OdometryDisplacementModel::Type parseDisplacementType(std::istream& in)
{
  std::string line;
  std::getline(in, line);
  if (line == "ProportionalXY")
    return Leph::OdometryDisplacementModel::Type::DisplacementProportionalXY;
  if (line == "ProportionalXYA")
    return Leph::OdometryDisplacementModel::Type::DisplacementProportionalXYA;
  if (line == "LinearSimpleXY")
    return Leph::OdometryDisplacementModel::Type::DisplacementLinearSimpleXY;
  if (line == "LinearSimpleXYA")
    return Leph::OdometryDisplacementModel::Type::DisplacementLinearSimpleXYA;
  if (line == "LinearFullXY")
    return Leph::OdometryDisplacementModel::Type::DisplacementLinearFullXY;
  if (line == "LinearFullXYA")
    return Leph::OdometryDisplacementModel::Type::DisplacementLinearFullXYA;
...
Leph::OdometryNoiseModel::Type parseNoiseType(std::istream& in)
{
  ...
  std::getline(in, line);
  if (line == "Constant")
    return Leph::OdometryNoiseModel::Type::NoiseConstant;
  if (line == "Proportional")
    return Leph::OdometryNoiseModel::Type::NoiseProportional;
  if (line == "LinearSimple")
    return Leph::OdometryNoiseModel::Type::NoiseLinearSimple;
  if (line == "LinearFull")
    return Leph::OdometryNoiseModel::Type::NoiseLinearFull;
...
void readLearningConfig(const std::string& filename, struct LearningConfig* conf)
{
  ...
  input >> conf->sampling_number >> conf->learning_set_size >> conf->cmaes_elitism >> conf->cmaes_max_iterations >>
      conf->cmaes_restarts >> conf->cmaes_lambda >> conf->cmaes_sigma;
  input.close();
}
 ...
 */
Eigen::VectorXd evaluateParameters(const Eigen::VectorXd& params, const Leph::OdometrySequence& data, bool noRandom,
                                   Leph::Odometry& odometry, std::default_random_engine& engine)
{
  (void)params;

  simulateOdometry(data, noRandom, odometry, conf.odometry_type, engine, nullptr);
...
void writeResultLine(std::ostream& out, double learningScore, double validationScore, size_t* logId)
{
  if (logId != nullptr)
  {
    out << "NA," << (*logId) << ",";
  ...
  out << conf.displacement_type << "," << conf.noise_type << "," << learningScore << "," << validationScore
      << std::endl;
}
...
void writeParamsLine(std::ostream& out, const Eigen::VectorXd& params, size_t* logId)
{
  if (logId != nullptr)
  {
    out << "NA," << (*logId) << ",";
...
void runLearning(const std::vector<Leph::OdometrySequence>& logs, std::ostream& result_output,
                 std::ostream& params_output, size_t* logId)
{
  ...
  Leph::LogLikelihoodMaximization<Leph::OdometrySequence, Leph::Odometry> calibration;
  ...
  if (logId != nullptr)
  {
    ...
    calibration.runOptimization(conf.sampling_number, learningSet, validationSet, conf.cmaes_max_iterations,
                                conf.cmaes_restarts, conf.cmaes_lambda, conf.cmaes_sigma, conf.cmaes_elitism, nullptr);
  }
  else
  {
    calibration.runOptimization(conf.sampling_number, conf.learning_set_size, conf.cmaes_max_iterations,
                                conf.cmaes_restarts, conf.cmaes_lambda, conf.cmaes_sigma, conf.cmaes_elitism, nullptr);
  }
...
void diagnose(const std::vector<Leph::OdometrySequence>& logs, std::ostream& result_output, std::ostream& params_output)
{
...
void classic(const std::vector<Leph::OdometrySequence>& logs, std::ostream& result_output, std::ostream& params_output)
{
  runLearning(logs, result_output, params_output, nullptr);
}

void ascending(const std::vector<Leph::OdometrySequence>& logs, std::ostream& result_output,
...
int main(int argc, char** argv)
{
  ...
  {
    std::cout << "Usage: ./app "
              << "odometry.data learningConfig.conf outputPrefix" << std::endl;
    return 1;
  }
  ...
  std::cout << "Loading odometry data from: " << logPath << " with " << logs.size() << " sequences" << std::endl;
