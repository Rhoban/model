...
void displaySequence(const OdometryData& data, size_t index)
{
  ...
  {
    plot.add(Leph::VectorLabel("order_x", data.walkTrajsOrder[index][i].x(), "order_y",
                               data.walkTrajsOrder[index][i].y(), "order_theta", data.walkTrajsOrder[index][i].z(),
                               "walk_phase", data.walkTrajsPhase[index][i], "read_x", data.readTrajsPose[index][i].x(),
                               "read_y", data.readTrajsPose[index][i].y(), "goal_x", data.goalTrajsPose[index][i].x(),
 ...
 */
double odometryModelFitness(const OdometryData& data, OdometryType type, Leph::OdometryDisplacementModel::Type model,
                            const Eigen::VectorXd parameters, size_t indexStart, size_t indexEnd, bool isLearningScore,
                            Leph::Plot* plot = nullptr)
{
  ...
  for (size_t i = indexStart; i <= indexEnd; i++)
  {
    ...
    if (plot != nullptr)
    {
      ...
      plot->add(Leph::VectorLabel("target_x", 0.0, "target_y", 0.0));
    }
    ...
    for (size_t j = 0; j < data.readTrajsPose[i].size(); j++)
    {
      ...
      if (type == OdometryOrder)
      {
        if (lastPhase > 0.8 && phase < 0.2)
        {
          ...
          odometry.updateFullStep(2.0 * data.walkTrajsOrder[i][j].segment(0, 3) * enableGain, nullptr);
        }
      }
      else if (type == OdometryGoal)
      {
        odometry.update(data.goalTrajsPose[i][j], data.goalTrajsSupport[i][j], nullptr);
      }
      else if (type == OdometryRead)
      {
        odometry.update(data.readTrajsPose[i][j], data.readTrajsSupport[i][j], nullptr);
      }
      else
      {
      ...
      if (plot != nullptr)
      {
        odometryDebugRead->update(data.readTrajsPose[i][j], data.readTrajsSupport[i][j], nullptr);
        odometryDebugGoal->update(data.goalTrajsPose[i][j], data.goalTrajsSupport[i][j], nullptr);
        if (lastPhase > 0.8 && phase < 0.2)
        {
          double enableGain = data.walkTrajsOrder[i][j](3);
          odometryDebugOrder->updateFullStep(2.0 * data.walkTrajsOrder[i][j].segment(0, 3) * enableGain, nullptr);
        }
        plot->add(Leph::VectorLabel(
            "odometry_x", odometry.state().x(), "odometry_y", odometry.state().y(), "identityRead_x",
            odometryDebugRead->state().x(), "identityRead_y", odometryDebugRead->state().y(), "identityGoal_x",
            odometryDebugGoal->state().x(), "identityGoal_y", odometryDebugGoal->state().y(), "identityOrder_x",
            odometryDebugOrder->state().x(), "identityOrder_y", odometryDebugOrder->state().y()));
      }
      lastPhase = phase;
    }
    if (plot != nullptr)
    {
      plot->add(
          Leph::VectorLabel("target_x", data.targetDisplacements[i].x(), "target_y", data.targetDisplacements[i].y()));
      std::cout << "DistIdentityRead="
                << (data.targetDisplacements[i].segment(0, 2) - odometryDebugRead->state().segment(0, 2)).norm()
                << " DistIdentityGoal="
                << (data.targetDisplacements[i].segment(0, 2) - odometryDebugGoal->state().segment(0, 2)).norm()
                << " DistIdentityOrder="
                << (data.targetDisplacements[i].segment(0, 2) - odometryDebugOrder->state().segment(0, 2)).norm()
                << " DistParameters="
                << (data.targetDisplacements[i].segment(0, 2) - odometry.state().segment(0, 2)).norm() << std::endl;
    ...
    if (isLearningScore)
    {
      double tmpDist = distanceFromArc(odometry.state().x(), odometry.state().y(), data.targetDisplacements[i].x(),
                                       data.targetDisplacements[i].y(), AngularRangeFitness);
      ...
      error += pow(tmpDist, 2);
      if (data.targetDisplacements[i].z() >= 0.0 && errorAngle > 2.0 * M_PI / 12.0)
      {
    ...
    {
      error += (data.targetDisplacements[i].segment(0, 2) - odometry.state().segment(0, 2)).squaredNorm();
    }
    count++;
  }
  if (count == 0)
 ...
 */
Eigen::VectorXd defaultParameters(Leph::OdometryDisplacementModel::Type model)
{
  Leph::Odometry odometry(model);
 ...
 */
Eigen::VectorXd odometryModelOptimization(const OdometryData& data, OdometryType type,
                                          Leph::OdometryDisplacementModel::Type model, size_t indexStart,
                                          size_t indexEnd)
{
  // Fitness function
  libcmaes::FitFuncEigen fitness = [&data, &type, &model, &indexStart, &indexEnd](const Eigen::VectorXd& parameters) {
    return odometryModelFitness(data, type, model, parameters, indexStart, indexEnd, true);
  };

  // Initial parameters
  Eigen::VectorXd initParams = defaultParameters(model);
  // CMAES initialization
  libcmaes::CMAParameters<> cmaparams(initParams, -1.0, CMAESLambda);
...
int main(int argc, char** argv)
{
  ...
  if (std::string(argv[1]) == "RUN")
  {
    ...
    std::cerr << "Special run -- Method=" << argMethodName << " StartCount=" << argStartCount << std::endl;
    indexFile = 4;
  }
  ...
  if (IsPrintCSV)
  {
    std::cout << "OdometryModelName,CountLearn,CountTest,Try,RESULTS,InitLearn,Learn,InitTest,Test" << std::endl;
    std::cout << "OdometryModelName,CountLearn,CountTest,Try,TEST,Index,InitTest,Test" << std::endl;
    std::cout << "OdometryModelName,CountLearn,CountTest,Try,PARAMS,SizeParams,..." << std::endl;
  }
  ...
  for (const auto& it : models)
  {
    ...
    for (size_t i = StartNumberLearnSeqs; i <= learnEndIndex; i += IncrNumberLearnSeqs)
    {
      ...
      for (size_t k = 0; k < NumberTries; k++)
      {
        ...
        Eigen::VectorXd bestParams = odometryModelOptimization(*dataLearn, TypeOdometry, it.second, learnStartIndex, i);
        // Compute RMSE score on learning and validation set
        double scoreInitialLearning =
            odometryModelFitness(*dataLearn, TypeOdometry, it.second, initParams, learnStartIndex, i, true);
        double scoreLearning =
            odometryModelFitness(*dataLearn, TypeOdometry, it.second, bestParams, learnStartIndex, i, true);
        double scoreInitialValidation =
            odometryModelFitness(*dataTest, TypeOdometry, it.second, initParams, testStartIndex, testEndIndex, false);
        double scoreValidation =
            odometryModelFitness(*dataTest, TypeOdometry, it.second, bestParams, testStartIndex, testEndIndex, false);
        // Print CSV
        if (IsPrintCSV)
        {
          // Print summary
          std::cout << it.first << "," << i - learnStartIndex + 1 << "," << testEndIndex - testStartIndex + 1 << ","
                    << k << ",RESULTS"
                    << "," << scoreInitialLearning << "," << scoreLearning << "," << scoreInitialValidation << ","
                    << scoreValidation << std::endl;
          // Evaluate each validation sequence independendly
          // without computing the RMSE
          for (size_t j = testStartIndex; j <= testEndIndex; j++)
          {
            double scoreInitialValidationSingle =
                odometryModelFitness(*dataTest, TypeOdometry, it.second, initParams, j, j, false);
            double scoreValidationSingle =
                odometryModelFitness(*dataTest, TypeOdometry, it.second, bestParams, j, j, false);
            std::cout << it.first << "," << i - learnStartIndex + 1 << "," << testEndIndex - testStartIndex + 1 << ","
                      << k << ",TEST"
                      << "," << j - testStartIndex + 1 << "," << scoreInitialValidationSingle << ","
                      << scoreValidationSingle << std::endl;
          }
        ...
        else
        {
          ...
          std::cout << it.first << " nbLearn=" << i - learnStartIndex + 1
                    << " nbTest=" << testEndIndex - testStartIndex + 1 << " try=" << k
                    << " initLearn=" << scoreInitialLearning << " learn=" << scoreLearning
                    << " initTest=" << scoreInitialValidation << " test=" << scoreValidation
                    << " dim=" << bestParams.size() << " " << (scoreInitialValidation > scoreValidation ? "" : "!!!")
                    << " Params:" << std::endl
                    << bestParams << std::endl;
        }
        ...
        if (IsDebug)
        {
          for (size_t j = testStartIndex; j <= testEndIndex; j++)
          {
            ...
            Leph::Plot plot;
            odometryModelFitness(*dataTest, TypeOdometry, it.second, bestParams, j, j, false, &plot);
            plot.plot("odometry_x", "odometry_y", Leph::Plot::LinesPoints, "index")
