...
void usage()
{
  std::cout << "Usage: ./app robothost [goal|read|corrected] [sigmaban|sigmaban_plus|grosban]" << std::endl;
  exit(1);
}
...
int main(int argc, char** argv)
{
  ...
  if (mode != "goal" && mode != "read" && mode != "corrected")
  {
    usage();
  }
  ...
  std::cout << (type == Leph::SigmabanModel ? "Sigmaban" : "Grosban") << " model" << std::endl;
  ...
  clientSub.setHandlerFloat([&streamedValues](const std::string name, int64_t timestamp, double val) {
    (void)timestamp;
  ...
  clientSub.setHandlerInt([&mutexModel, &model, &prefix](const std::string name, int64_t timestamp, int val) {
  ...
  clientSub.setHandlerBool([&mutexVar, &walkEnable](const std::string name, int64_t timestamp, bool val) {
  ...
  while (viewer.update())
  {
    ...
    viewer.drawText(fieldPos + Eigen::AngleAxisd(fieldOrientation, Eigen::Vector3d(0.0, 0.0, 1.0)).toRotationMatrix() *
    ...
    viewer.drawSphere(ballPos + Eigen::Vector3d(0.0, 0.0, 0.075), 0.075, ballColor, ballColor, ballColor);
    viewer.drawText(ballPos + Eigen::Vector3d(0.0, 0.0, 0.25), 15, ssBall.str(), 1.0, 1.0, 1.0);
    // Draw walk controls
    Eigen::Vector3d pose = model.get().getPose();
    Eigen::Vector3d pos(pose.x(), pose.y(), 0.0);
    Eigen::Matrix3d rot = Eigen::AngleAxisd(pose.z(), Eigen::Vector3d(0.0, 0.0, 1.0)).toRotationMatrix();
    ...
    if (walkEnable)
    {
      viewer.drawCylinder(pos + Eigen::Vector3d(0.0, 0.0, -0.02), 0.1, 0.02, 0.0, 1.0, 0.0);
    }
    else
    {
      viewer.drawCylinder(pos + Eigen::Vector3d(0.0, 0.0, -0.02), 0.1, 0.02, 1.0, 0.0, 0.0);
    }
    ...
    viewer.drawText(pos + rot * Eigen::Vector3d(walkStep / 100.0, 0.0, 0.0), 20, ssStep.str(), 1.0, 0.0, 0.0);
    viewer.drawText(pos + rot * Eigen::Vector3d(0.0, walkLateral / 100.0, 0.0), 20, ssLateral.str(), 0.0, 1.0, 0.0);
    viewer.drawText(pos + Eigen::Vector3d(0.0, 0.0, walkTurn / 100.0), 20, ssTurn.str(), 0.0, 0.0, 1.0);
    // Draw robot state
    viewer.drawText(model.get().position("camera", "origin") + Eigen::Vector3d(0.0, 0.0, 0.5), 25, stateRoboCup, 0.0,
                    1.0, 1.0);
    viewer.drawText(model.get().position("camera", "origin") + Eigen::Vector3d(0.0, 0.0, 0.35), 25, stateApproach, 1.0,
                    0.0, 1.0);
    viewer.drawText(model.get().position("camera", "origin") + Eigen::Vector3d(0.0, 0.0, 0.2), 25, stateLowlevel, 1.0,
                    1.0, 0.0);
