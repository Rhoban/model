...
OdometryDisplacementModel::OdometryDisplacementModel(Type type) : _type(type), _params(), _minBounds(), _maxBounds()
{
  ...
  else if (_type == DisplacementProportionalXY)
  {
    ...
    _params << 1.0,             // dX = dx
        1.0;                    // dY = dy
    _minBounds << IdentityMin,  // dX = dx
        IdentityMin;            // dY = dy
    _maxBounds << IdentityMax,  // dX = dx
        IdentityMax;            // dY = dy
  }
  ...
  else if (_type == DisplacementLinearSimpleXY)
  {
    ...
    _params << 0.0,                  // dX = offset
        1.0,                         // dX = dx
        0.0,                         // dY = offset
        1.0;                         // dY = dy
    _minBounds << -OffsetStepBound,  // dX = offset
        IdentityMin,                 // dX = dx
        -OffsetLateralBound,         // dY = offset
        IdentityMin;                 // dY = dy
    _maxBounds << OffsetStepBound,   // dX = offset
        IdentityMax,                 // dX = dx
        OffsetLateralBound,          // dY = offset
        IdentityMax;                 // dY = dy
  }
...
double OdometryDisplacementModel::setParameters(const Eigen::VectorXd& params)
{
  // Check size
  if (params.size() != _params.size())
  {
    throw std::logic_error("OdometryDisplacementModel invalid input size");
  }
...
std::vector<std::string> OdometryDisplacementModel::getParametersNames() const
{
  ...
  {
    return { "dx_bias", "dx_from_dx", "dx_from_dy", "dx_from_dz", "dy_bias", "dy_from_dx", "dy_from_dy", "dy_from_dz" };
  }
  else if (_type == DisplacementLinearFullXYA)
  {
...
Eigen::Vector3d OdometryDisplacementModel::displacementCorrection(const Eigen::Vector3d& diff) const
{
  ...
  {
    newDiff.x() = _params(0) * diff.x();
    newDiff.y() = _params(1) * diff.y();
  }
  else if (_type == DisplacementProportionalXYA)
  {
    newDiff.x() = _params(0) * diff.x();
    newDiff.y() = _params(1) * diff.y();
    newDiff.z() = _params(2) * diff.z();
  }
  else if (_type == DisplacementLinearSimpleXY)
  {
    newDiff.x() = _params(0) + _params(1) * diff.x();
    newDiff.y() = _params(2) + _params(3) * diff.y();
  }
  else if (_type == DisplacementLinearSimpleXYA)
  {
    newDiff.x() = _params(0) + _params(1) * diff.x();
    newDiff.y() = _params(2) + _params(3) * diff.y();
    newDiff.z() = _params(4) + _params(5) * diff.z();
  }
  else if (_type == DisplacementLinearFullXY)
  {
    newDiff.x() = _params(0) + _params(1) * diff.x() + _params(2) * diff.y() + _params(3) * diff.z();
    newDiff.y() = _params(4) + _params(5) * diff.x() + _params(6) * diff.y() + _params(7) * diff.z();
  }
  else if (_type == DisplacementLinearFullXYA)
  {
    newDiff.x() = _params(0) + _params(1) * diff.x() + _params(2) * diff.y() + _params(3) * diff.z();
    newDiff.y() = _params(4) + _params(5) * diff.x() + _params(6) * diff.y() + _params(7) * diff.z();
    newDiff.z() = _params(8) + _params(9) * diff.x() + _params(10) * diff.y() + _params(11) * diff.z();
  }
  else
  {
    throw std::logic_error("OdometryDisplacementModel invalid type");
...
void OdometryDisplacementModel::printParameters() const
{
  ...
  {
    std::cout << "Displacement dX = dx: " << _params(0) << std::endl;
    std::cout << "Displacement dY = dy: " << _params(1) << std::endl;
  }
  else if (_type == DisplacementProportionalXYA)
  {
    std::cout << "Displacement dX = dx: " << _params(0) << std::endl;
    std::cout << "Displacement dY = dy: " << _params(1) << std::endl;
    std::cout << "Displacement dA = da: " << _params(2) << std::endl;
  }
  else if (_type == DisplacementLinearSimpleXY)
  {
    std::cout << "Displacement dX = offset: " << _params(0) << std::endl;
    std::cout << "Displacement dX = dx: " << _params(1) << std::endl;
    std::cout << "Displacement dY = offset: " << _params(2) << std::endl;
    std::cout << "Displacement dY = dy: " << _params(3) << std::endl;
  }
  else if (_type == DisplacementLinearSimpleXYA)
  {
    std::cout << "Displacement dX = offset: " << _params(0) << std::endl;
    ...
    std::cout << "Displacement dY = offset: " << _params(2) << std::endl;
    std::cout << "Displacement dY = dy: " << _params(3) << std::endl;
    std::cout << "Displacement dA = offset: " << _params(4) << std::endl;
    std::cout << "Displacement dA = da: " << _params(5) << std::endl;
  }
  else if (_type == DisplacementLinearFullXY)
  {
    std::cout << "Displacement dX = offset: " << _params(0) << std::endl;
    std::cout << "Displacement dX = dx: " << _params(1) << std::endl;
    std::cout << "Displacement dX = dy: " << _params(2) << std::endl;
    std::cout << "Displacement dX = da: " << _params(3) << std::endl;
    std::cout << "Displacement dY = offset: " << _params(4) << std::endl;
    std::cout << "Displacement dY = dx: " << _params(5) << std::endl;
    std::cout << "Displacement dY = dy: " << _params(6) << std::endl;
    std::cout << "Displacement dY = da: " << _params(7) << std::endl;
  }
  else if (_type == DisplacementLinearFullXYA)
  {
    std::cout << "Displacement dX = offset: " << _params(0) << std::endl;
    ...
    std::cout << "Displacement dY = dy: " << _params(6) << std::endl;
    std::cout << "Displacement dY = da: " << _params(7) << std::endl;
    std::cout << "Displacement dA = offset: " << _params(8) << std::endl;
    std::cout << "Displacement dA = dx: " << _params(9) << std::endl;
    std::cout << "Displacement dA = dy: " << _params(10) << std::endl;
    std::cout << "Displacement dA = da: " << _params(11) << std::endl;
  }
  else
  {
    throw std::logic_error("OdometryDisplacementModel invalid type");
