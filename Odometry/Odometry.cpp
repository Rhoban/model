...
void Odometry::update(const Eigen::Vector3d& current, Leph::HumanoidFixedModel::SupportFoot supportFoot,
...
void Odometry::update(HumanoidFixedModel& model, std::default_random_engine* engine)
{
  update(model.get().getPose(), model.getSupportFoot(), engine);
}
...
void Odometry::saveToFile(const std::string& path) const
{
  ...
  file << (int)getDisplacementType() << " " << (int)getNoiseType() << std::endl;
...
void Odometry::loadFromFile(const std::string& path)
{
  ...
  file >> tmpTypeNoise;
  OdometryDisplacementModel::Type typeDisplacement = (OdometryDisplacementModel::Type)tmpTypeDisplacement;
  OdometryNoiseModel::Type typeNoise = (OdometryNoiseModel::Type)tmpTypeNoise;
  ...
  if (isError > 0.0)
  {
    ...
    oss << "Odometry parameters are out of bounds: " << isError << std::endl;
