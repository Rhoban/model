...
Vector3D::Vector3D(const Vector3D& other) : vector<double>(other)
{
}
...
Frame3D::Frame3D(const Frame3D& other) : vector<Vector3D>(other)
{
}
...
Frame3D Frame3D::from_euler(double psi, double theta, double phi)
{
  ...
  res[0] = Vector3D(cos(phi) * cos(psi) - sin(phi) * cos(theta) * sin(psi),
                    cos(phi) * sin(psi) + sin(phi) * cos(theta) * cos(psi), sin(phi) * sin(theta));
  res[1] = Vector3D(-sin(phi) * cos(psi) - cos(phi) * cos(theta) * sin(psi),
                    -sin(phi) * sin(psi) + cos(phi) * cos(theta) * cos(psi), cos(phi) * sin(theta));
  res[2] = Vector3D(sin(theta) * sin(psi), -sin(theta) * cos(psi), cos(theta));
  return res;
}
...
Position::Position(double theta0, double theta1, double theta2, double theta3, double theta4, double theta5)
{
  theta[0] = theta0;
  theta[1] = theta1;
  theta[2] = theta2;
...
bool IK::compute(Vector3D C, Frame3D orientation, Position& result, double* boundIKDistance, bool inverseKnee,
                 bool forceForward)
{
  if (boundIKDistance != nullptr)
  {
    *boundIKDistance = std::fabs(L[0]);
    *boundIKDistance = std::min(*boundIKDistance, std::fabs(L[1]));
  }
  ...
  if (boundIKDistance != nullptr)
  {
    *boundIKDistance = std::min(*boundIKDistance, std::fabs(B_len));
    *boundIKDistance = std::min(*boundIKDistance, -B[2]);
  }
  ...
  if (forceForward && !(phi_e1 > 0 || (is_zero(phi_e1) && scalar_prod(phi, e2) >= 0)))
  {
    phi = -1.0 * phi;
  }
  ...
  if (boundIKDistance != nullptr)
  {
    *boundIKDistance = std::min(*boundIKDistance, 1.0 - std::fabs(q));
  }
  ...
  Vector3D omega(-sin(result.theta[0]) * sin(result.theta[1]), cos(result.theta[0]) * sin(result.theta[1]),
                 -cos(result.theta[1]));
...
bool IKTest::test(double L0, double L1, double L2, Vector3D C, double euler_psi, double euler_theta, double euler_phi,
                  Position result, double epsilon)
{
  ...
  for (int i = 0; i < 6; i++)
    if (val_abs(res.theta[i] - result.theta[i]) > epsilon)
      return false;
  return true;
}
...
bool IKTest::test_suite()
{
  ...
  printf("-- Test 1: \n");
  LAUNCH(test(1.0, 1.0, 0.0, Vector3D(0.0, 0.0, -sqrt(2)), 0.0, 0.0, 0.0,
              Position(0.0, 0.0, DEG2RAD(45.0), DEG2RAD(90.0), DEG2RAD(45.0), 0.0), 0.01))

  printf("-- Test 2: \n");
  LAUNCH(test(1.0, 1.0, 0.2, Vector3D(0.0, 0.0, -2.2), 0.0, 0.0, 0.0,
              Position(DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0)), 0.01))

  printf("-- Test 3: \n");
  LAUNCH(test(1.0, 1.0, 0.2, Vector3D(0.0, 0.0, -2.2), DEG2RAD(15.0), 0.0, 0.0,
              Position(DEG2RAD(15.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(0.0)), 0.01))

  printf("-- Test 4: \n");
  LAUNCH(test(1.0, 1.0, 0.2, Vector3D(0.0, 0.0, -sqrt(2) - 0.2), DEG2RAD(45), 0.0, 0.0,
              Position(DEG2RAD(45), 0.0, DEG2RAD(45.0), DEG2RAD(90.0), DEG2RAD(45.0), 0.0), 0.01))
  ...
  printf("-- Test 6: \n");
  LAUNCH(test(1.0, 1.0, 0.2, Vector3D(0.0, 0.0, -sqrt(2) - 0.2), DEG2RAD(0), 0.0, 0.0,
              Position(DEG2RAD(0.0), DEG2RAD(0.0), DEG2RAD(45.0), DEG2RAD(90.0), DEG2RAD(45.0), DEG2RAD(0.0)), 0.01))
  return test_result;
}
