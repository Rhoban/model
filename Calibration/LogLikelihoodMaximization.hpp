...
public:
   ...
   * and assume no noise.
   */
  typedef std::function<Eigen::VectorXd(const Eigen::VectorXd& params, const TypeData&, bool noRandom, TypeModel& model,
   ...
   */
  typedef std::function<double(const Eigen::VectorXd& params)> BoundFunc;
   ...
   * functions.
   */
  typedef std::function<TypeModel(const Eigen::VectorXd& params)> InitFunc;
   ...
   * and estimation.
   */
  typedef std::function<Eigen::VectorXd(const Eigen::VectorXd& obs)> TransformFunc;
   ...
   */
  void setInitialParameters(const Eigen::VectorXd& initParams, const Eigen::VectorXd& normCoef)
  {
    _params = initParams;
    _normCoef = normCoef;
    if (_params.size() != _normCoef.size())
    {
      throw std::logic_error("LogLikelihoodMaximization invalid params size");
    }
    for (size_t i = 0; i < (size_t)_normCoef.size(); i++)
    {
      if (_normCoef(i) <= 0.0)
      {
        throw std::logic_error("LogLikelihoodMaximization negative or null normCoef");
      }
    }
  }
   ...
   */
  void setUserFunctions(EvalFunc func1, BoundFunc func2, InitFunc func3, TransformFunc func4 = TransformFunc())
  {
    _evalFunc = func1;
    _boundFunc = func2;
    _initFunc = func3;
   ...
   */
  void addObservation(const Eigen::VectorXd& obs, const TypeData& data)
  {
    // Check size
    if (_observations.size() > 0 && _observations.front().size() != obs.size())
    {
      throw std::logic_error("LogLikelihoodMaximization invalid size");
    }
  ...
  void runOptimization(unsigned int samplingNumber, double learningDataRatio, unsigned int maxIterations,
                       unsigned int restart, unsigned int populationSize = 10, double sigma = -1.0,
                       unsigned int elitismLevel = 0, Leph::Plot* plot = nullptr)
  {
    // Check ratio
    if (learningDataRatio <= 0.0 || learningDataRatio >= 1.0)
    {
      throw std::logic_error("LogLikelihoodMaximization invalid learning ratio");
    }

    // Shuffle learning and testing set
    unsigned int learningSize = std::floor(learningDataRatio * _observations.size());
    runOptimization(samplingNumber, learningSize, maxIterations, restart, populationSize, sigma, elitismLevel, plot);
  }
   ...
   */
  void runOptimization(unsigned int samplingNumber, unsigned int learningSize, unsigned int maxIterations,
                       unsigned int restart, unsigned int populationSize = 10, double sigma = -1.0,
                       unsigned int elitismLevel = 0, Leph::Plot* plot = nullptr)
  {
    // Check size
    ...
    {
      throw std::logic_error("LogLikelihoodMaximization not enough observations");
    }
    if (learningSize <= 0)
    {
      throw std::logic_error("LogLikelihoodMaximization invalid learning size: " + std::to_string(learningSize));
    }
    unsigned int validationSize = _observations.size() - learningSize;
    if (validationSize < 2)
    {
      throw std::logic_error("RunOptimization require at least 2 validation observations");
    }
    std::vector<size_t> learningIndices, testIndices;
    dataDispatch(learningSize, _observations.size(), learningIndices, testIndices);
    runOptimization(samplingNumber, learningIndices, testIndices, maxIterations, restart, populationSize, sigma,
                    elitismLevel, plot);
  }

  void runOptimization(unsigned int samplingNumber, const std::vector<size_t>& learningIndices,
                       const std::vector<size_t>& testIndices, unsigned int maxIterations, unsigned int restart,
                       unsigned int populationSize = 10, double sigma = -1.0, unsigned int elitismLevel = 0,
                       Leph::Plot* plot = nullptr)
  {
    ...
    // Fitness function
    libcmaes::FitFuncEigen fitness = [this](const Eigen::VectorXd& params) {
      return this->scoreFitness(params.array() * this->_normCoef.array(), false);
    };

    // Progress function
    libcmaes::ProgressFunc<libcmaes::CMAParameters<>, libcmaes::CMASolutions> progress =
        [this, &iterations, plot](const libcmaes::CMAParameters<>& cmaparams, const libcmaes::CMASolutions& cmasols) {
          // Empty case
          ...
          Eigen::VectorXd params = this->_normCoef.array() * cmasols.get_best_seen_candidate().get_x_dvec().array();
          if (iterations % 10 == 0)
          {
            double meanLearn;
            double varLearn;
            ...
            double minTest;
            double maxTest;
            this->testParameters(params, meanLearn, varLearn, minLearn, maxLearn, meanTest, varTest, minTest, maxTest);
            double scoreLearn = this->scoreFitness(params, false);
            double scoreTest = this->scoreFitness(params, true);
            std::cout << "============" << std::endl;
            std::cout << "Iteration=" << iterations << " dim=" << params.size()
                      << " LearnSize=" << this->_indexesLearning.size() << " TestSize=" << this->_indexesTesting.size()
                      << std::endl;
            std::cout << "Learn: score=" << scoreLearn << " mean=" << meanLearn << " stdDev=" << std::sqrt(varLearn)
                      << " min=" << minLearn << " max=" << maxLearn << std::endl;
            std::cout << "Test : score=" << scoreTest << " mean=" << meanTest << " stdDev=" << std::sqrt(varTest)
                      << " min=" << minTest << " max=" << maxTest << std::endl;
            std::cout << "============" << std::endl;
            if (plot != nullptr)
            {
              plot->add({
    ...
    libcmaes::CMAParameters<> cmaparams(_params.array() / _normCoef.array(), sigma, populationSize);
    ...
    // Retrieve best Trajectories and score
    _params = cmasols.get_best_seen_candidate().get_x_dvec().array() * _normCoef.array();
    std::cout << "Iterations: " << iterations << std::endl;
    std::cout << "Dimensions: " << _params.size() << std::endl;
    std::cout << "LearnSize: " << _indexesLearning.size() << " TestSize: " << _indexesTesting.size() << std::endl;
    std::cout << "BestParams: " << _params.transpose() << std::endl;
    std::cout << "BestScoreLearning: " << scoreFitness(_params, false) << std::endl;
    std::cout << "BestScoreTesting:  " << scoreFitness(_params, true) << std::endl;
    double meanLearn;
    double varLearn;
    double meanTest;
    double varTest;
    ...
    double minTest;
    double maxTest;
    testParameters(_params, meanLearn, varLearn, minLearn, maxLearn, meanTest, varTest, minTest, maxTest);
    std::cout << "IterationsNumber: " << iterations << std::endl;
    std::cout << "MeanLearn: " << meanLearn << " stdDevLearn: " << std::sqrt(varLearn) << " min: " << minLearn
              << " max: " << maxLearn << std::endl;
    std::cout << "MeanTest:  " << meanTest << " stdDevTest:  " << std::sqrt(varTest) << " min: " << minTest
              << " max: " << maxTest << std::endl;
  }
   ...
   */
  void testParameters(const Eigen::VectorXd& params, double& meanLearn, double& varLearn, double& minLearn,
                      double& maxLearn, double& meanTest, double& varTest, double& minTest, double& maxTest) const
  {
    ...
    for (size_t i = 0; i < _indexesLearning.size(); i++)
    {
      ...
      Eigen::VectorXd estimate = _evalFunc(params, _dataContainer[indexSet], true, model, engine);
      ...
      if (_transformFunc)
      {
        ...
        // before comparison
        Eigen::VectorXd error = _transformFunc(estimate) - _transformFunc(_observations[indexSet]);
        cmpError = error.norm();
      }
      else
      {
    ...
    meanLearn = sumErrorLearn / (double)_indexesLearning.size();
    meanTest = sumErrorTest / (double)_indexesTesting.size();
    varLearn = sum2ErrorLearn / (double)_indexesLearning.size() - pow(meanLearn, 2);
    varTest = sum2ErrorTest / (double)_indexesTesting.size() - pow(meanTest, 2);
  }
   ...
   */
  double scoreFitness(const Eigen::VectorXd& params, bool useTestData) const
  {
    const std::vector<size_t>& usedSet = useTestData ? _indexesTesting : _indexesLearning;
    ...
    for (size_t i = 0; i < usedSet.size(); i++)
    {
      ...
      double tmpScore =
          scoreParametersLogLikelihood(params, _observations[indexSet], _dataContainer[indexSet], model, engine);
...
private:
   ...
   */
  double scoreParametersLogLikelihood(const Eigen::VectorXd& params, const Eigen::VectorXd& obs, const TypeData& data,
                                      TypeModel& model, std::default_random_engine& engine) const
  {
    ...
    {
      // Call user function
      Eigen::VectorXd estimate = _evalFunc(params, data, false, model, engine);
      // Check size
      if (estimate.size() != obs.size())
      {
        throw std::logic_error("LogLikelihoodMaximization " + std::string("user function invalid size"));
      }
      // Save it
   ...
   */
  void dataDispatch(size_t learningSize, size_t nbObservations, std::vector<size_t>& learningIndexes,
                    std::vector<size_t>& testingIndexes)
  {
    ...
    {
      throw std::logic_error("LogLikelihoodMaximization invalid learning size");
    }
    // Random device initialization
    std::random_device rd;
    std::default_random_engine engine(rd());
