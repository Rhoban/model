...
namespace Leph
{
ModelViewer::ModelViewer(unsigned int width, unsigned int height)
  : _width(width)
  , _height(height)
  , _window(sf::VideoMode(width, height), "OpenGL", sf::Style::Default, sf::ContextSettings(32))
  , _font()
  , _camPos(0.0, 0.0, 0.0)
...
bool ModelViewer::update(bool freeFly)
{
  ...
  if (mousePosition.x < 0 || mousePosition.y < 0 || mousePosition.x >= (int)_window.getSize().x ||
  ...
  if (freeFly)
  {
    ...
    _camView = Eigen::AngleAxisd(-camViewVel * mouseDeltaPosY, camLat).toRotationMatrix() * _camView;
    _camView = Eigen::AngleAxisd(camViewVel * mouseDeltaPosX, camUp).toRotationMatrix() * _camView;
    _camView.normalize();
  ...
  else
  {
    ...
    if (_camAngle2 < 0.01)
      _camAngle2 = 0.01;
    if (_camAngle2 > M_PI - 0.01)
      _camAngle2 = M_PI - 0.01;
    if (_camRadius <= 0.0)
      _camRadius += wheelDelta * camPosVel * 20.0;
...
void ModelViewer::drawFrame(const Eigen::Vector3d& center, const Eigen::Matrix3d& orientation)
{
  glPushMatrix();
...
void ModelViewer::drawMass(const Eigen::Vector3d& center, const Eigen::Matrix3d& orientation, double color)
{
  glPushMatrix();
  glTranslated(center.x(), center.y(), center.z());
  applyRotation(orientation.transpose());
...
void ModelViewer::drawJoint(const Eigen::Vector3d& center, const Eigen::Matrix3d& orientation, double color)
{
  glPushMatrix();
  glTranslated(center.x(), center.y(), center.z());
  applyRotation(orientation.transpose());
...
void ModelViewer::drawLink(const Eigen::Vector3d& pt1, const Eigen::Vector3d& pt2, double color)
{
...
void ModelViewer::drawBox(double sizeX, double sizeY, double sizeZ, const Eigen::Vector3d& center,
                          const Eigen::Matrix3d& orientation, double r, double g, double b)
{
  glPushMatrix();
  glTranslated(center.x(), center.y(), center.z());
  applyRotation(orientation.transpose());
...
void ModelViewer::drawSphere(const Eigen::Vector3d& center, double radius, double r, double g, double b)
{
  glPushMatrix();
...
void ModelViewer::drawCylinder(const Eigen::Vector3d& base, double radius, double height, double r, double g, double b)
{
  glPushMatrix();
  glLineWidth(3.0 * groundThickness);
  glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
void ModelViewer::drawArrow(const Eigen::Vector3d& center, const Eigen::Vector3d& vect, double length, double r,
                            double g, double b)
{
  ...
  Eigen::Vector3d sideVect1 =
      Eigen::AngleAxisd(M_PI / 4.0, Eigen::Vector3d(0.0, 0.0, 1.0)).toRotationMatrix() * vect.normalized();
...
void ModelViewer::drawLine(const Eigen::Vector3d& start, const Eigen::Vector3d& end, double thick, double r, double g,
                           double b)
{
  glPushMatrix();
...
void ModelViewer::drawText(const Eigen::Vector3d& position, unsigned int size, const std::string& str, double r,
                           double g, double b)
{
  // Compute 3d position on screen
  Eigen::Vector2d pixel = getPointProjection(position);
  if (pixel.x() < 0.0 || pixel.x() >= _width || pixel.y() < 0.0 || pixel.y() >= _height)
  {
    return;
  }
...
void ModelViewer::addTrackedPoint(const Eigen::Vector3d& point, Color color)
{
  if (color == Red)
  {
...
void ModelViewer::updateCamera(bool useViewVector)
{
  ...
  glLoadIdentity();
  gluLookAt(_camPos.x(), _camPos.y(), _camPos.z(), viewPoint.x(), viewPoint.y(), viewPoint.z(), 0.0, 0.0, 1.0);
}
...
void ModelViewer::drawCube(float r, float g, float b, bool isWireFrame)
{
  if (isWireFrame)
  {
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
void ModelViewer::drawColorTrajectory(const std::list<Eigen::Vector3d>& traj, double r, double g, double b)
{
  if (traj.size() == 0)
  {
    return;
  ...
  glBegin(GL_POINTS);
  glColor3f(r, g, b);
  glVertex3f(traj.front().x(), traj.front().y(), traj.front().z());
  glEnd();
...
Eigen::Vector2d ModelViewer::getPointProjection(const Eigen::Vector3d& pos) const
{
  // Requested 3d point
  GLdouble x = pos.x();
  GLdouble y = pos.y();
  ...
  GLint success = gluProject(x, y, z, model, projection, viewport, &wx, &wy, &wz);
