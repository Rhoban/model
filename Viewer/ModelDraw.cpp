...
void ModelDraw(Model& model, ModelViewer& viewer, double color)
{
  ...
  for (size_t i = 0; i < model.sizeFrame(); i++)
  {
    ...
    viewer.drawBox(sizeX, sizeY, sizeZ, pos + mat.transpose() * center, mat);
  }

  // Draw center of mass
  viewer.drawMass(model.centerOfMass("origin"), RBDLMath::Matrix3d::Identity(), color);
  ...
  for (size_t i = 0; i < rbdlModel.mBodies.size(); i++)
  {
    ...
    if (rbdlModel.mBodies[rbdlModel.lambda[i]].mMass > 0.0001)
    {
      ...
      {
        transformAxis =
            Eigen::AngleAxisd(-M_PI / 2.0, jointAxis.cross(RBDLMath::Vector3d(1.0, 0.0, 0.0))).toRotationMatrix();
      }
      if ((jointAxis - RBDLMath::Vector3d(-1.0, 0.0, 0.0)).norm() < 0.001)
      {
        transformAxis *= -1.0;
...
void CameraDraw(const CameraModel& cameraModel, HumanoidModel& model, ModelViewer& viewer)
{
  ...
  model.cameraViewVectorToWorld(model.cameraPixelToViewVector(cameraModel, Eigen::Vector2d(0, 0)), groundPos1);
  model.cameraViewVectorToWorld(model.cameraPixelToViewVector(cameraModel, Eigen::Vector2d(maxX, 0)), groundPos2);
  model.cameraViewVectorToWorld(model.cameraPixelToViewVector(cameraModel, Eigen::Vector2d(maxX, maxY)), groundPos3);
  model.cameraViewVectorToWorld(model.cameraPixelToViewVector(cameraModel, Eigen::Vector2d(0, maxY)), groundPos4);
  model.cameraViewVectorToWorld(model.cameraPixelToViewVector(cameraModel, Eigen::Vector2d(maxX / 2, maxY / 2)),
                                groundPos5);
...
{
  double thick = 5.0;
  Eigen::Matrix3d rot = Eigen::AngleAxisd(yaw, Eigen::Vector3d(0.0, 0.0, 1.0)).toRotationMatrix();
...
void CleatsDraw(HumanoidSimulation& simulation, ModelViewer& viewer)
{
  std::vector<std::string> names = {
    "left_cleat_1",  "left_cleat_2",  "left_cleat_3",  "left_cleat_4",
    "right_cleat_1", "right_cleat_2", "right_cleat_3", "right_cleat_4",
  };
  for (const std::string& name : names)
  {
    ...
    if (force >= 0.0)
    {
      viewer.drawCylinder(pos, 0.002, 0.01 * force, 1.0, 1.0, 1.0);
    }
    else
    {
      viewer.drawCylinder(pos + Eigen::Vector3d(0.0, 0.0, 0.01 * force), 0.002, -0.01 * force, 1.0, 0.0, 0.0);
    }
  }
}
